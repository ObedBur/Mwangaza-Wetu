...
async function migrate() {
  ...
  try {
      ...
      await prisma.administrateur.upsert({
        ...
        create: {
          ...
          dateCreation: new Date(
            admin.date_creation || admin.created_at || Date.now(),
          ),
        },
      });
    }
      ...
      await prisma.membre.upsert({
        ...
        create: {
          ...
          dateNaissance: membre.date_naissance
            ? new Date(membre.date_naissance)
            : null,
    ...
    for (const ep of epargnes) {
      // On vérifie si le membre existe pour la relation facultative
      const memberExists = await prisma.membre.findUnique({
        where: { numeroCompte: ep.compte },
      });
