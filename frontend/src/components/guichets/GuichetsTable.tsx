...
import CashierRow from "@/components/guichets/CashierRow";

const DEFAULT_PAGE_SIZE = 10;

export default function GuichetsTable() {
  ...
  const cashiersList = response?.data ?? [];
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-red-100 dark:border-red-900/30 p-16 flex flex-col items-center justify-center text-center text-sm">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-xs mb-6">
          {error instanceof Error
            ? error.message
            : "Impossible de récupérer les caissiers pour le moment."}
        </p>
        <button
          onClick={() => refetch()}
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800">
              {cashiersList.length === 0 ? (
                ...
                cashiersList.map((cashier) => (
                  <CashierRow key={cashier.id} cashier={cashier} />
                ))
              )}
            </tbody>
          </table>
